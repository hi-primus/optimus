...
from optimus.spark.audf import abstract_udf as audf, filter_row_by_data_type as fbdt
# Helpers
from optimus.spark.helpers.check import is_num_or_str, is_list, is_, is_tuple, is_list_of_dataframes, is_list_of_tuples, \
    is_function, is_one_element, is_type, is_int, is_str, has_, is_column_a, is_dataframe
from optimus.spark.helpers.columns import get_output_cols, parse_columns, check_column_numbers, validate_columns_names, \
    name_col
from optimus.spark.helpers.columns_expression import match_nulls_strings, match_null, zeros_agg, hist_agg, count_na_agg, \
    percentile_agg, count_uniques_agg, range_agg
from optimus.spark.helpers.constants import PYSPARK_NUMERIC_TYPES, PYTHON_TYPES, PYSPARK_NOT_ARRAY_TYPES, \
    PYSPARK_STRING_TYPES, PYSPARK_ARRAY_TYPES
from optimus.spark.helpers.converter import one_list_to_val, tuple_to_dict, format_dict, val_to_list
from optimus.spark.helpers.functions import append as append_df
from optimus.spark.helpers.functions import filter_list, collect_as_list, create_buckets
from optimus.spark.helpers.parser import parse_python_dtypes, parse_spark_class_dtypes, parse_col_names_funcs_to_keys, \
    ...
    class Cols:
        ...
        def select(columns="*", regex=None, data_type=None, invert=False):
            ...
            if columns is not None:
                ...
                df.set_meta(value=source_df.get_meta())
                result = df
            else:
                result = None
        ...
        def copy(input_cols, output_cols):
            ...
            :return:
            """
            input_cols = parse_columns(source_df, input_cols)
        ...
        def apply(input_cols, func=None, func_return_type=None, args=None, func_type=None, when=None,
            ...
            input_cols = parse_columns(source_df, input_cols, filter_by_column_dtypes=filter_col_by_dtypes,
            ...
            # df = df.track_cols(source_df, output_cols)
        ...
        def rename(columns_old_new=None, func=None):
            ...
            if is_function(func):
                ...
                validate_columns_names(source_df, columns_old_new)
                for col_name in columns_old_new:
                    old_col_name = col_name[0]
                    if is_str(old_col_name):
                        df = df.withColumnRenamed(old_col_name, col_name[1])
                    elif is_int(old_col_name):
                        df = df.withColumnRenamed(source_df.schema.names[old_col_name], col_name[1])
            df.set_meta(value=source_df.get_meta())
        ...
        def move(column, position, ref_col=None):
            ...
            column = parse_columns(source_df, column)
            ref_col = parse_columns(source_df, ref_col)
        ...
        def keep(columns=None, regex=None):
            ...
            if regex:
                r = re.compile(regex)
                columns = list((r.match, source_df.columns))
        ...
        def drop(columns=None, regex=None, data_type=None):
            ...
            columns = parse_columns(source_df, columns, filter_by_column_dtypes=data_type)
        ...
        def exec_agg(exprs):
            ...
            :param exprs: Columns Expression to run
            :return:
            """
            ...
            if len(exprs) > 0:
                ...
                if ENGINE == "sql":
                    ...
                    df = source_df.query(exprs)
                elif ENGINE == "spark":
                    df = source_df.agg(*exprs)
        ...
        def percentile(columns, values=None, relative_error=RELATIVE_ERROR):
            ...
            return Cols.agg_exprs(columns, percentile_agg, source_df, values, relative_error)
        ...
        def mad(columns, relative_error=RELATIVE_ERROR, more=None):
            ...
            for col_name in columns:
                ...
                _mad = {}
                median_value = source_df.cols.median(col_name, relative_error)
                mad_value = source_df.withColumn(col_name, F.abs(F.col(col_name) - median_value)) \
        ...
        def mode(columns):
            ...
            for col_name in columns:
                count = source_df.groupBy(col_name).count()
                mode_df = count.join(
        ...
        def remove(columns, search=None, search_by="chars", output_cols=None):
            ...
            :return:
            """
            return source_df.cols.replace(columns, search, "", search_by, output_cols)
        ...
        def remove_special_chars(input_cols, output_cols=None):
            ...
            input_cols = parse_columns(source_df, input_cols, filter_by_column_dtypes=PYSPARK_STRING_TYPES)
            check_column_numbers(input_cols, "*")

            df = source_df.cols.replace(input_cols, [s for s in string.punctuation], "", "chars",
                                        output_cols=output_cols)
            return df
        ...
        def replace(input_cols, search=None, replace_by=None, search_by="chars", output_cols=None):
            ...
            def func_words(_df, _input_col, _output_col, _search, _replace_by):
                ...
                data_type = source_df.cols.dtypes(_input_col)
        ...
        def fill_na(input_cols, value=None, output_cols=None):
            ...
            for input_col, output_col in zip(input_cols, output_cols):
                func = None
                if is_column_a(source_df, input_col, PYSPARK_NUMERIC_TYPES):
                    new_value = fast_float(value)
                    func = F.when(match_nulls_strings(input_col), new_value).otherwise(F.col(input_col))
                elif is_column_a(source_df, input_col, PYSPARK_STRING_TYPES):
                    new_value = str(value)
                    func = F.when(match_nulls_strings(input_col), new_value).otherwise(F.col(input_col))
                elif is_column_a(source_df, input_col, PYSPARK_ARRAY_TYPES):
        ...
        def is_na(input_cols, output_cols=None):
            ...
            return source_df.cols.apply(input_cols, _replace_na, output_cols=output_cols)
        ...
        def count_na(columns):
            ...
            return format_dict(Cols.agg_exprs(columns, count_na_agg, source_df))
        ...
        def value_counts(columns):
            ...
            result = {}
            for col_name in columns:
                result.update(compress_dict(source_df.groupBy(col_name).count().orderBy('count').to_dict(), col_name))
            return result
        ...
        def unique(columns):
            ...
            result = {}
            for col_name in columns:
                result.update(compress_list(source_df.select(col_name).distinct().to_dict()))
            return result
        ...
        def nunique(*args, **kwargs):
            ...
            :return:
            """
            return Cols.count_uniques(*args, **kwargs)
        ...
        def select_by_dtypes(data_type):
            ...
            columns = parse_columns(source_df, '*', is_regex=None, filter_by_column_dtypes=data_type)
        ...
        def z_score(input_cols, output_cols=None):
            ...
            def _z_score(col_name, attr):
                mean_value = source_df.cols.mean(col_name)
                stdev_value = source_df.cols.std(col_name)
        ...
        def iqr(columns, more=None, relative_error=RELATIVE_ERROR):
            ...
            quartile = source_df.cols.percentile(columns, [0.25, 0.5, 0.75], relative_error=relative_error)
            for col_name in columns:
        ...
        def nest(input_cols, shape="string", separator="", output_col=None):
            ...
            if shape is "vector":
                input_cols = parse_columns(source_df, input_cols, filter_by_column_dtypes=PYSPARK_NUMERIC_TYPES)
                # check_column_numbers(input_cols, ">1")
            ...
            for input_col, output_col in zip(input_cols, output_cols):
                ...
                if is_column_a(df, input_col, StructType):
                    ...
                    if infer_splits is True:
                        splits = format_dict(source_df.agg(F.max(F.size(input_col))).to_dict())
                ...
                # String
                    ...
                    if infer_splits is True:
                        splits = format_dict(
                            source_df.agg(F.max(F.size(F.split(F.col(input_col), separator)))).to_dict())
                ...
                # df = df.track_cols(source_df, output_col)
                else:
        ...
        def cell(column):
            ...
            :return:
            """
            return source_df.cols.select(column).first()[0]
        ...
        def scatter(columns, buckets=10):
            ...
            for col_name in columns:
                ...
                df = Cols.bucketizer(df, col_name, splits=splits, output_cols=name_col(col_name, "bucketizer"))
        ...
        def hist(columns, buckets=20):
            ...
            result = Cols.agg_exprs(columns, hist_agg, source_df, buckets)
            ...
            # df = source_df
            # for col_name in columns:
            #     print(agg_exprs(hist_agg, col_name, source_df, buckets))
            #     # print(df.agg(hist_agg(col_name, source_df, buckets)))
            # return result
        ...
        def schema_dtype(columns="*"):
            ...
            """
            columns = parse_columns(source_df, columns)
            return format_dict([source_df.schema[col_name].dataType for col_name in columns])
        ...
        def dtypes(columns="*"):
            ...
            columns = parse_columns(source_df, columns)
            data_types = tuple_to_dict(source_df.dtypes)
        ...
        def names(col_names="*", filter_by_column_dtypes=None, invert=False):
            ...
            :return:
            """
            columns = parse_columns(source_df, col_names, filter_by_column_dtypes=filter_by_column_dtypes,
                                    invert=invert)
            return columns
        ...
        def meta(self, col_name):
            ...
            result = ""
            meta_json = source_df._jdf.schema().json()
        ...
        def meta(self, col_name, value):
            ...
            return source_df.withColumn(col_name, F.col(col_name).alias(col_name, metadata=value))
