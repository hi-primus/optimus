...
def string_to_index(df, input_cols, output_cols=None, columns=None, **kargs):
    ...
    df.meta.set(value=df.meta.preserve(df_actual, Actions.STRING_TO_INDEX.value, output_cols).get())
...
def index_to_string(df, input_cols, output_cols=None, columns=None, **kargs):
    ...
    df.meta.set(value=df.meta.preserve(df_actual, Actions.INDEX_TO_STRING.value, output_cols).get())
