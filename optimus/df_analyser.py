...
# Input data:
### Use this https://github.com/kxxoling/PTable to print tables
class GeneralDescripTable:
    def __init__(self,  column_number, row_number):
        ...
        self.dat = [column_number, row_number]
...
class DataFrameAnalyzer:
    ...
    def _analyze(self, df_col_analyzer, column, row_number, plots, print_type, values_bar, num_bars, types_dict):
        ...
        # Calling verification routine to obtain datatype's counts
        ...
        return invalid_cols, percentages, numbers, 1
    ...
    def general_description(self):
        ...
        temp_obj = GeneralDescripTable(col_number, row_number)
        display(temp_obj)

        data_types = len(set([x[1] for x in self._df.dtypes]))

        return self._create_dict(["filename", "size", "columns", "rows", "datatypes"],
                                 ["a", 500, col_number, row_number, data_types])

    # Funtion to analize column datatypes, it also plot proportion datatypes and histograms:
    def column(self, columns, plots=True, values_bar=True, print_type=False, num_bars=10, print_all=False):
        """
        Return the counts the number of registers in a column that are numbers (integers, floats) and the number of
        string registers.
        :param columns: a list or a string column names
        :param plots: a list or a string column names
        :param values_bar: Can be True or False. If it is True, frequency values are placed over each bar.
        :param print_type: Can be True or False. If true it will print the analysis.
        :param num_bars: number of bars printed in histogram
    ...
    def get_numerical_hist(self, df_one_col, num_bars):
        """
        This function analyzes a column dataframe (only numerical columns) and
        returns a dictionary with bins and values of frequency."""
