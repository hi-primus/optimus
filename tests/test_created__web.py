...
class TestWebPandas(TestBase):
    ...
    def test_cols_strip_html(self):
        df = self.create_dataframe(dict={('strip_html_test', 'object'): ['<this is a test>', '<this>2 < 3, 2 <= 2, 3 > 2</this> </is> <a/> <test>', '<this> is a </test>', '<this is> a <test>', '<>this is a test<>', '>this is a test<']}, force_data_types=True)
        result = df.cols.strip_html(cols=['strip_html_test'])
        expected = self.create_dataframe(dict={('strip_html_test', 'object'): ['', '2  2   ', ' is a ', ' a ', 'this is a test', '>this is a test<']}, force_data_types=True)
        self.assertTrue(result.equals(expected, decimal=True, assertion=True))
