...
class Testdf_outliers(object):
	...
	def test_outliers_tukey_info():
		...
		actual_df =json_enconding(actual_df)
		expected_value =json_enconding({'count_outliers': 7, 'count_non_outliers': 3, 'lower_bound': -6.5, 'upper_bound': 45.5, 'iqr1': 13, 'iqr3': 26})
		assert (expected_value == actual_df)
	@staticmethod
	...
	def test_outliers_tukey_whiskers():
		...
		actual_df =json_enconding(actual_df)
		expected_value =json_enconding({'lower_bound': -6.5, 'upper_bound': 45.5, 'iqr1': 13, 'iqr3': 26})
		assert (expected_value == actual_df)
	@staticmethod
