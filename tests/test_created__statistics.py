...
class TestStatisticsPandas(TestBase):
    ...
    def test_cols_mad_all(self):
        df = self.df
        result = df.cols.mad(cols='*')
        expected = {'NullType': nan, 'attributes': nan, 'date arrival': nan, 'function(binary)': nan, 'height(ft)': 9.0, 'japanese name': nan, 'last date seen': nan, 'last position seen': nan, 'rank': 1.0, 'Cybertronian': 0.0, 'Date Type': 5.12784e+16, 'age': 0.0, 'function': nan, 'names': nan, 'timestamp': 0.0, 'weight(t)': 1.7000000000000002}
        self.assertEqual(json_encoding(result), json_encoding(expected))
    ...
    def test_cols_max_all(self):
        ...
        expected = {'NullType': nan, 'attributes': '[None, 5700.0]', 'date arrival': '1980/04/10', 'function(binary)': "bytearray(b'Security')", 'height(ft)': 300.0, 'japanese name': "['Roadbuster']", 'last date seen': '2016/09/10', 'last position seen': '37.789563,-122.400356', 'rank': 10, 'Cybertronian': True, 'Date Type': Timestamp('2016-09-10 00:00:00'), 'age': 5000000, 'function': 'Security', 'names': 'ironhide&', 'timestamp': Timestamp('2014-06-24 00:00:00'), 'weight(t)': 5.7}
        self.assertEqual(json_encoding(result), json_encoding(expected))
    ...
    def test_cols_min_all(self):
        ...
        expected = {'NullType': nan, 'attributes': '[3.9624, 1800.0]', 'date arrival': '1980/04/10', 'function(binary)': "bytearray(b'Battle Station')", 'height(ft)': -28.0, 'japanese name': "['Bumble', 'Goldback']", 'last date seen': '2011/04/10', 'last position seen': '10.642707,-71.612534', 'rank': 7, 'Cybertronian': False, 'Date Type': Timestamp('2011-04-10 00:00:00'), 'age': 5000000, 'function': 'Battle Station', 'names': 'Jazz', 'timestamp': Timestamp('2014-06-24 00:00:00'), 'weight(t)': 1.8}
        self.assertEqual(json_encoding(result), json_encoding(expected))
    ...
    def test_cols_mode_all(self):
        ...
        expected = {'NullType': [], 'attributes': [], 'date arrival': [], 'function(binary)': [], 'height(ft)': [-28.0, 13.0, 17.0, 26.0, 300.0], 'japanese name': [], 'last date seen': [], 'last position seen': [], 'rank': [7.0, 8.0, 10.0], 'Cybertronian': 1.0, 'Date Type': [1.3023936e+18, 1.336608e+18, 1.372032e+18, 1.403568e+18, 1.4391648e+18, 1.4734656e+18], 'age': 5000000.0, 'function': [], 'names': [], 'timestamp': 1.403568e+18, 'weight(t)': [1.8, 2.0, 4.0, 4.3, 5.7]}
        self.assertEqual(json_encoding(result), json_encoding(expected))
    
    def test_cols_mode_multiple(self):
        df = self.df
        result = df.cols.mode(cols=['height(ft)', 'age', 'rank'])
        expected = {'height(ft)': [-28.0, 13.0, 17.0, 26.0, 300.0], 'age': 5000000.0, 'rank': [7.0, 8.0, 10.0]}
        self.assertEqual(json_encoding(result), json_encoding(expected))
    ...
    def test_cols_std_all(self):
        df = self.df
        result = df.cols.std(cols='*')
        expected = {'NullType': nan, 'attributes': nan, 'date arrival': nan, 'function(binary)': nan, 'height(ft)': 132.66612227694, 'japanese name': nan, 'last date seen': nan, 'last position seen': nan, 'rank': 1.3662601021279464, 'Cybertronian': 0.408248290463863, 'Date Type': 6.385771726480677e+16, 'age': 0.0, 'function': nan, 'names': nan, 'timestamp': 0.0, 'weight(t)': 1.6471186963907611}
        self.assertEqual(json_encoding(result), json_encoding(expected))
    ...
    def test_cols_var_all(self):
        df = self.df
        result = df.cols.var(cols='*')
        expected = {'NullType': nan, 'attributes': nan, 'date arrival': nan, 'function(binary)': nan, 'height(ft)': 17600.3, 'japanese name': nan, 'last date seen': nan, 'last position seen': nan, 'rank': 1.8666666666666665, 'Cybertronian': 0.16666666666666669, 'Date Type': 4.0778080542720004e+33, 'age': 0.0, 'function': nan, 'names': nan, 'timestamp': 0.0, 'weight(t)': 2.713}
        self.assertEqual(json_encoding(result), json_encoding(expected))
