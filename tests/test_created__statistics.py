...
class TestStatisticsPandas(TestBase):
 ...
 'Cybertronian', 'age', 'function', 'names', 'timestamp', 'weight(t)'])
    ...
    def test_cols_mode_all(self):
        ...
        expected = {'NullType': [], 'attributes': [[91.44, None], [8.5344, 4300.0], [None, 5700.0], [7.9248, 4000.0], [3.9624, 1800.0], [5.334, 2000.0]], 'date arrival': '1980/04/10', 'function(binary)': [bytearray(b'Battle Station'), bytearray(b'Espionage'), bytearray(b'First Lieutenant'), bytearray(b'Leader'), bytearray(b'None'), bytearray(b'Security')], 'height(ft)': [-28.0, 13.0, 17.0, 26.0, 300.0], 'japanese name': [['Bumble', 'Goldback'], ['Inochi', 'Convoy'], ['Megatron'], ['Meister'], ['Metroflex'], ['Roadbuster']], 'last date seen': ['2011/04/10', '2012/05/10', '2013/06/10', '2014/07/10', '2015/08/10', '2016/09/10'], 'last position seen': ['10.642707,-71.612534', '19.442735,-99.201111', '33.670666,-117.841553', '37.789563,-122.400356'], 'rank': [7, 8, 10], 'Cybertronian': True, 'Date Type': [datetime.datetime(2011, 4, 10, 0, 0), datetime.datetime(2012, 5, 10, 0, 0), datetime.datetime(2013, 6, 24, 0, 0), datetime.datetime(2014, 6, 24, 0, 0), datetime.datetime(2015, 8, 10, 0, 0), datetime.datetime(2016, 9, 10, 0, 0)], 'age': 5000000, 'function': ['Battle Station', 'Espionage', 'First Lieutenant', 'Leader', 'None', 'Security'], 'names': ['Jazz', 'Megatron', 'Metroplex_)^$', 'Optimus', 'bumbl#ebéé  ', 'ironhide&'], 'timestamp': datetime.datetime(2014, 6, 24, 0, 0), 'weight(t)': [1.8, 2.0, 4.0, 4.3, 5.7]}
        self.assertTrue(results_equal(result, expected, decimal=5, assertion=True))
    ...
    def test_cols_std_all(self):
        df = self.df
        result = df.cols.std(cols=['NullType', 'attributes', 'date arrival', 'function(binary)', 'height(ft)',
 'japanese name', 'last date seen', 'last position seen', 'rank',
 'Cybertronian', 'age', 'function', 'names', 'timestamp', 'weight(t)'])
    ...
    def test_cols_var_all(self):
        df = self.df
        result = df.cols.var(cols=['NullType', 'attributes', 'date arrival', 'function(binary)', 'height(ft)',
 'japanese name', 'last date seen', 'last position seen', 'rank',
 'Cybertronian', 'age', 'function', 'names', 'timestamp', 'weight(t)'])
